...
fetch("https://api.ipify.org/?format=json")
    .then((res) => res.json())
    .then((data) => {
      text.innerText ="Your Current IP Address is"+" " +data.ip;
    })
      ...
      fetch(`https://ipinfo.io/${ipAddress}?token=a65aa5704d8782`)
        ...
        .then((data) => {
          ...
          const pincode = data.postal;
           document.getElementById("Ipadress").innerHTML=`<h3>IP Address :${ip}</h3>`
...
function showLocationOnMap(lat, lon, data) {
  ...
  // Display the map
  mapDiv.innerHTML =` <h1>Your Current Location</h1>${mapUrl}` ;
}
